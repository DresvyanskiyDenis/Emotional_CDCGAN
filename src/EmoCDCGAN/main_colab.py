...
def train():
    ...
    for train_step in range(train_steps):
        ...
        for i in range(len(indexes_to_choose)):
            img=real_images[i]
            # TODO: now all images are resized, but had no cropping. In future, before form batches, crop it
            img=preprocess_image(img, scale=True, resize=False, needed_shape=(224,224,3), bgr=False)
            real_images[i]=img
