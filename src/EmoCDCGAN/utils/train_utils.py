...
def train_n_mini_batches(model, data, labels, num_mini_batches, batch_size=4,
    ...
    # print('vars:',train_vars[0])
    accum_gradient = [tf.zeros_like(this_var) for this_var in train_vars]
    total_loss = 0
    for i in range(num_mini_batches // batch_size):
    ...
    batch_loss = total_loss / (num_mini_batches//batch_size)
    if model.name == 'discriminator':
...
if __name__ == "__main__":
    ...
    model.add(
        tf.keras.layers.Conv2D(32, 4, strides=(1, 1), activation='relu', padding='same', input_shape=x_train.shape[1:]))
    ...
    for i in range(1000):
        ...
        batch_loss = train_n_mini_batches(model=model, data=x_train[:32], labels=y_train[:32], num_mini_batches=8,
                                          batch_size=4,
                                          loss=tf.keras.losses.categorical_crossentropy)
