...
def train():
    path_to_data='C:\\Users\\Dresvyanskiy\\Downloads\\archive\\Batches_celebA'
    ...
    # generator model
    generator_model=simpleGAN.create_generator(dropout_rate=0.2)
    #generator_model.load_weights('saved_models/generator.h5')

    # discriminator model
    discriminator_model=simpleGAN.create_discriminator(dropout_rate=0.2)
    ...
    for train_step in range(0,train_steps,1):
        ...
        if train_step % validate_each_step == 0:
            ...
            visualize_images(images=generated_images, labels=[i for i in range(generated_images.shape[0])], path_to_save='images', save_name='generated_images_step_%i.png'%train_step)
